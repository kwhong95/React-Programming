...
## 7.4.4 플러그인 제작하기
> 플러그인은 웹팩의 처리 과정을 이해해야 작성 가능하기에 로더보다 작성하기 까다로움  
> `DefinePlugin` 처럼 모듈의 내용도 수정 가능하기 때문에 로더가 할 수 있는 거의 모든일을 수행 가능 > 강력크함

#### 웹팩이 생성하는 번들 파일의 목록과 각 파일의 크기 정보를 파일로 저장해주는 플러그인 제작하기
> `webpack-custom-plugin` 코드 참조